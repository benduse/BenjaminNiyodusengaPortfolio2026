...
export const projects = [
  {
    ...
    tagline: "A developer resource-sharing forum where engineers submit, discuss, and upvote tools worth knowing about.",
    problem:
    ...
    role:
      "Built the full application end-to-end — from data modeling in Supabase through the React interface, deployment, and testing — as the sole developer during a CodePath web development engagement.",
    ...
    challenges: [
      {
        challenge: "Filter state needed to survive page reloads and be shareable, not just live in component state.",
        solution: "Moved filter state into the URL via search params, so the UI reads from and writes to the URL instead of local state alone.",
      },
      {
        challenge: "Letting users post images and links without a heavyweight upload pipeline.",
        solution: "Used Supabase Storage directly from the client for image upload, and wrote a lightweight parser to detect and embed YouTube links automatically.",
      },
      {
        challenge: "Keeping the UI consistent as the app grew past a handful of components.",
        solution: "Introduced a design-token system (spacing, color, and type scale) so new components stay visually consistent without copy-pasting styles.",
      },
    ],
    technologies: ["React", "JavaScript", "Supabase", "PostgreSQL", "Playwright", "Netlify", "CSS"],
    github: "",
    liveDemo: "",
  },
  {
    ...
    title: "MoodHabit",
    category: ["Full Stack", "React"],
    tagline: "A mood-adaptive habit tracker that adjusts its suggestions based on how you're actually feeling that day.",
    problem:
    ...
    role:
      "Designed the data model, built the rule engine that maps mood to habit suggestions, and implemented the UI and animation layer.",
    architecture: "React 18 + TypeScript → local rule engine → localStorage / Supabase dual data layer",
    features: [
    ...
    challenges: [
      {
        challenge: "Habit logic needed to feel personal without becoming a black box.",
        solution: "Wrote the suggestion logic as an explicit, readable rule engine rather than a model, so behavior stays predictable and easy to extend.",
      },
      {
        challenge: "Supporting both offline use and cross-device sync without duplicating logic.",
        solution: "Built a dual data layer that reads/writes through a shared interface, backed by localStorage or Supabase depending on connection state.",
      },
    ],
  ...
  },
  {
    ...
    tagline: "A Python and SQL data pipeline that turns raw call-center logs into structured business reports.",
    problem:
    ...
    role:
      "Built the data infrastructure and analytics engine solo as a Data Engineer Trainee: import, transformation, querying, and reporting layers.",
    architecture: "CSV files → Python ingestion layer → in-memory SQLite → SQL analytics → CSV reports",
    features: [
    ...
    challenges: [
      {
        challenge: "Raw call logs arrived in inconsistent formats that could break naive imports.",
        solution: "Built a defensive CSV ingestion layer with validation before rows are written into SQLite, so bad records fail loudly instead of corrupting the dataset.",
      },
      {
        challenge: "Turning SQL query results into something a non-technical manager could use.",
        solution: "Wrote a reporting layer that exports query output into clean, structured CSV reports rather than raw query dumps.",
      },
    ],
  ...
  },
  {
    ...
    tagline: "A Kinyarwanda flashcard app with fuzzy answer matching and streak tracking, built to make daily practice stick.",
    problem:
    ...
    challenges: [
      {
        challenge: "Exact-string matching penalized users for minor typos or accent differences.",
        solution: "Implemented fuzzy string matching so answers within a small edit-distance threshold are accepted, without letting genuinely wrong answers slip through.",
      },
      {
        challenge: "Keeping review sessions from feeling repetitive as the deck grew.",
        solution: "Added a mastered-cards system that removes well-known cards from regular rotation, keeping sessions focused on what still needs practice.",
      },
    ],
  ...
  },
  {
    ...
    tagline: "A reading-data dashboard that pulls live catalog data from the Open Library API and visualizes it.",
    problem:
    ...
    challenges: [
      {
        challenge: "API responses were inconsistent in shape and occasionally incomplete.",
        solution: "Added normalization logic between the API layer and the UI so components always receive a predictable shape, regardless of gaps in the source data.",
      },
    ],
  ...
  },
  {
    ...
    title: "VeniVici",
    category: ["Frontend", "React"],
    tagline: "A country-discovery app that turns exploring world geography and culture into a game.",
    problem:
